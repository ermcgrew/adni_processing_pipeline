...
## Using 2023 version of stats: 
# T1 SUV pet (not SUVR)
# 8mm PET scans
# whole brain seg NOT propagated to ANTS space


# Usage:
# ./pet_stats.sh id mridate taudate amydate  
# wholebrainseg wholebrainseg wblabelfile
# icv.txt 
# cleanup/seg_left cleanup/seg_right cleanup/seg_both
# t1tau t2taureg t1amyreg t2amyreg 
# stats_output_file 
# ASHST1dir/final/${id}  
...
stats_output_file=${15}

t1_ashs_seg_prefix=${16}
...
for side in left right; do
  for ((i=0;i<${#WBTAULABELIDS[*]}; i++)); do
    if [[ -f $wholebrainseg && -f $t1tau ]]; then
      if [[ $side == "left" ]]; then
      ...
      WBTAU="$WBTAU,$(c3d $wholebrainseg -replace $REPRULE -thresh 999 999 1 0 -as SEG \
        $t1tau -int 0 -reslice-identity \
...
### In Long's code, make SUVR T1 first before calculating -- don't use this data for now
#### ASHST1-tau measures
...
for side in left right; do
  ...
  for ((i=0;i<${#ASHST13TTAULABELIDS[*]}; i++)); do
    ...
    TAU="$TAU,$(c3d $SEG -replace $REPRULE -thresh 99 99 1 0 -as SEG \
      $t1tau -int 0 -reslice-identity -push SEG -lstat | awk '{print $2}' | tail -n 1)"
  done 
done
TAU="${TAU:1}"
