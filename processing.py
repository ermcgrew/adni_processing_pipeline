...
def run_status(id,date,input_file,output_file,processing_step):
    if file_exists(output_file):
        ...
        if file_exists(input_file):
            ...
            logging.info(f"{id}:{date}: No input file, cannot run {processing_step}")
            return False

if run_status('3452','2020-11-11','file','file', 'brainx'):
    print("we should run this")
...
class MRI:
    ...
    def wb_seg(self, parent_job_name = ""):
        wait = wait_value(parent_job_name)
        this_job_name=f"brainx_{self.date_id_prefix}"
        if file_exists(self.T1_wb_seg):
            logging.info(f"{self.id}:{self.mridate}: Whole Brain Segmentation already done.")
            return
        else:
    ...
    def t1_super_res(self, parent_job_name = ""):
        wait = wait_value(parent_job_name)
    ...
    def t1_ashs(self, parent_job_name = ""):
        wait = wait_value(parent_job_name)
    ...
    def t1_ashs_multitemplate_thickness(self, parent_job_name = ""):
        wait = wait_value(parent_job_name)
        for side in sides:
            if side == "left":
    ...
    def t1_ashs_icv(self, parent_job_name = ""):
        wait = wait_value(parent_job_name)
    ...
    def t2_ashs(self, parent_job_name = ""):
        wait = wait_value(parent_job_name)
    ...
    def t1_flair_reg(self, parent_job_name = ""):
        wait = wait_value(parent_job_name)
        ...
        if file_exists(self.T1_flair):
            ...
            if file_exists(self.flair) and file_exists(self.T1_trim):               
                ...
                os.system(f"bsub -o {self.filepath} -J {this_job_name} {wait} \
                          ./wrapper_scripts/t1_flair_reg.sh {self.T1_trim} {self.flair} {self.T1_flair}")
                return(this_job_name)
            else:
    ...
    def wmh_prep(self):
        if file_exists(self.wmh):
            ...
            if file_exists(self.flair):
                ...
                logging.info(f"{self.id}:{self.mridate}: collectinng flair files to send to lambda-picsl")
...
# jobname=MRIprocessing.t1_ashs()
# os.system(f"bjobs -J {jobname}")
# MRIprocessing.t1_ashs_multitemplate_thickness("t1_ashs_2020-10-27_141_S_6779")
